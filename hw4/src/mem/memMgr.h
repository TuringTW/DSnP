...
#define toSizeT(t)      SIZE_T*(t/SIZE_T+(size_t)(t%SIZE_T>0?1:0))  // TODO
//
...
class MemBlock
{
   ...
   // Constructor/Destructor     colon mean init the var before Constructor
...
public:
   ...
   void  freeArr(T* p) {
      ...
      size_t n = *(size_t*)p;
      MemRecycleList<T>* MRList = getMemRecycleList(n);
      MRList->pushFront(p);
      #ifdef MEM_DEBUG
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(temp->_arrSize!=n){
        if (temp->_nextList) {
          ...
          MemRecycleList<T>* newReList = new MemRecycleList<T>(n);
          temp->_nextList = newReList;
          return newReList;
        }
      }
      return temp;
   ...
   T* getMem(size_t t) {
      ...
      MemRecycleList<T>* MemRList = getMemRecycleList(n);
      if (MemRList->numElm()>0) {
        ret = MemRList->popFront();
        ...
        size_t _remaint = downtoSizeT(_activeBlock->getRemainSize());
        if (_remaint>=S) {
          n = getArraySize(_remaint);
          MemRList = getMemRecycleList(n);
          T* remain_ret;
          _activeBlock->getMem(n*S+SIZE_T, remain_ret);
          MemRList->pushFront(remain_ret);
        }
        //    #ifdef MEM_DEBUG
