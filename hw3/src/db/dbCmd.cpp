...
DBAppendCmd::exec(const string& option)
{
   ...
   vector<int> d;
   if (!lexOptions(option, options, 0))
   ...
   if (myStrNCmp("-Row", options[0], 2)==0) {
     ...
     for (size_t i = 0; i < dbtbl.nCols(); i++) {
       int temp;
       if(options.size()<=i+1||options[i+1]=="-") d.push_back(INT_MAX);
       else if (!myStr2Int(options[i+1], temp)){
         cerr << "Error: \"" << options[i+1] << "\" is not an integer!!" << endl;
         return CMD_EXEC_ERROR;
       }else d.push_back(temp);
     }
     dbtbl.addRow(DBRow(d));
     ...
     for (size_t i = 0; i < dbtbl.nRows(); i++) {
       int temp;
       if(options.size()<=i+1||options[i+1]=="-") d.push_back(INT_MAX);
       else if (!myStr2Int(options[i+1], temp)) return CMD_EXEC_ERROR;
       else d.push_back(temp);
     }
     dbtbl.addCol(d);
   }
...
DBPrintCmd::exec(const string& option)
{
   ...
   int row_num, col_num;
    ...
    if (options.size()==0) return CmdExec::errorOption(CMD_OPT_MISSING, "");
      ...
      if(options.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[1]);
      else if(options.size()<2) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
      if (checkRowIdx(options[1], row_num)){
        cout << dbtbl[row_num];
      }
    }else if(myStrNCmp("-Col", options[0], 2)==0) {
      if(options.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[1]);
      else if(options.size()<2) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
      if (checkColIdx(options[1], col_num)){
        dbtbl.printCol(col_num);
      }
    }else if (options.size()==2) {
      if(checkRowIdx(options[0], row_num)&&checkColIdx(options[1], col_num)){
        if(options.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[1]);
        dbtbl.printData(cout, dbtbl[row_num][col_num], true);
      }
